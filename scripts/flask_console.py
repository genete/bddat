...
class FlaskControlGUI:
    def __init__(self, root):
        ...
        network_frame.pack(pady=2)

        self.serve_network = tk.BooleanVar(value=False)
        self.chk_network = tk.Checkbutton(
            network_frame,
            text="Servir a la red local  (0.0.0.0 — compañeros pueden acceder)",
            variable=self.serve_network,
            font=("Arial", 9),
            fg="#444444",
        )
    ...
    def parse_ansi_colors(self, text):
        ansi_map = {
            ...
            "91": "bright_red",    "92": "bright_green", "93": "bright_yellow",
            "94": "bright_blue",   "95": "bright_magenta","96": "bright_cyan",
        }
    ...
    def insert_colored_text(self, text):
        for segment_text, tag in self.parse_ansi_colors(text):
    ...
    def start_server(self):
        ...
        self.status_label.config(
            text=f"Estado: Iniciando servidor... [{host_label}]",
            fg="orange",
        )
        ...
        self.log_area.insert(tk.END, f"Directorio base: {BDDAT_DIR}\n", "yellow")
        self.log_area.insert(tk.END, f"Host: {host}  ({host_label})\n", "yellow")
        self.log_area.insert(tk.END, "=" * 60 + "\n\n", "cyan")

        os.chdir(BDDAT_DIR)

        python_cmd = VENV_PYTHON if os.path.exists(VENV_PYTHON) else "python"
        venv_msg = "venv\\Scripts\\python.exe" if python_cmd == VENV_PYTHON else "Python global del sistema"
        self.log_area.insert(tk.END, f"Usando: {venv_msg}\n\n", "green")
        ...
        try:
            ...
            self.status_label.config(
                text=f"Estado: Servidor en ejecución  [{host_label}]",
                fg="green",
            )
