...
def cargar_estructura():
    ...
    return version, fases, tramites_lista


def ejecutar(version, fases, tramites_lista):
    ...
    return n_fases, n_tram


app = create_app()
with app.app_context():
    print(f"Cargando {JSON_PATH}...")
    version, fases, tramites_lista = cargar_estructura()
    print(f"  FTT v{version}: {len(fases)} fases, {len(tramites_lista)} trámites únicos")

    modo = 'FULL (operativos + maestros)' if FULL else 'maestros FTT'
    print(f"Ejecutando reset [{modo}]...")
    n_fases, n_tram = ejecutar(version, fases, tramites_lista)
