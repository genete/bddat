...
**Último cerrado:** **#559** — **Inspector-detalle del listado de seguimiento**: click en fila → inspector overlay (ADR-023) con el detalle del agregado en el **lenguaje del árbol** (semáforo por nodo, reutiliza `arbol_expediente.construir_arbol_solicitud`); edición delegada al árbol vía "Ir a tramitar" al **cuello de botella**. Paso 1 de unificación de color: tokens `--sem-*` al `:root` del shell (`v2-theme.css`). PR #569. Spin-offs: **#570** (filtros de listados v2 en URL), **#571** (el bundle del árbol hereda los tokens semáforo del shell).

**Actuales:** **#501** — Vista "Mi trabajo" del administrativo (ADR-017): agregado que navega al árbol (gemelo de seguimiento), con **inspector-detalle del agregado** (lectura) y edición delegada al árbol. Desbloqueado (#558 cerrado); patrón validado en #559.

**Próximos:** hilo UI — **#501** (Mi trabajo del administrativo, en curso) → **Mi trabajo del supervisor** (por crear). Después, bloque **escritos / motor adaptativo (M4)**: **#555** (clasificación ESFT de plantillas) → **#556** (variables del motor en plantillas) → **#561** (drop de `catalogo_variables.activa` + red de tests).
...
12. **#501 — ADR-017 Vista "Mi trabajo" del administrativo.** **En curso (Actual).** Agregado que navega al árbol (gemelo de seguimiento), con **inspector-detalle del agregado** (lectura) y edición delegada al árbol. Desbloqueado (#558 cerrado); patrón validado en #559. ADR-017 candidato a revisión. Requiere tabla unificada (#533) + scroll infinito. ~2-3 semanas.
