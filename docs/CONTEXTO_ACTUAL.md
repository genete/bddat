...
**Actuales:** **Mi trabajo del supervisor** — gemelo de #501 para el rol supervisor. **Issue por crear.**

**Próximos:** **Mi trabajo del supervisor** (por crear, ahora actual). Después, bloque **escritos / motor adaptativo (M4)**: **#555** (clasificación ESFT de plantillas) → **#556** (variables del motor en plantillas) → **#561** (drop de `catalogo_variables.activa` + red de tests).
...
14. **Mi trabajo del supervisor.** **Actual.** Gemelo de #501 para el rol supervisor (reusa cola/inspector). Issue por crear.
