...
- **La emisión del certificado de cierre de la solicitud quedó fuera y no tiene issue.** Se implementó el ancla (`solicitudes.documento_cierre_id` + `CERT_CIERRE_SOLICITUD`), no quién lo crea ni cuándo: mientras tanto una solicitud nunca alcanza `CUMPLIDO` y acabará marcándose vencida aunque se resolviera a tiempo. Decisión consciente (ADR-041 §D bis), pendiente de convertir en issue.
