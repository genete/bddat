...
**Próximo:** **#684** — el `POST .../nodo/tarea/<id>/requerimientos/catalogo` (#440) solo exige `gestionar_tarea`, así que cualquier TRAMITADOR/ADMINISTRATIVO inserta filas en `catalogo_requerimientos` desde el checkbox "Guardar en catálogo" del shuttle, eludiendo el CRUD restringido a SUPERVISOR+ADMIN de #593. Obligatorio: re-gatear (o retirar) el endpoint y renombrar el checkbox a "Solicitar guardado en catálogo". La parte deseable —notificar al Supervisor en vez de crear la fila— depende de #28 (M5, sin implementar): si no está, se hace solo la parte obligatoria, sin bloquear el fix de integridad. Sale del foco fijo de `ANÁLISIS_SOLICITUD` a propósito. En la cola: **#766** (ayuda en UI de la regla de recepción de #764, barato, dentro del foco) y, no crítico, **#743** (idea de seguimiento). Aplazados: #698 (poblado puro de catálogo), #572 (ortogonal, diferido por Carlos), #444/#555 (plantillas — cubierto por la base ya creada en #726, no urge). Sin issue a propósito: el guardián "el resultado de fase debe reflejar el diagnóstico" corresponde a la revisión de la fase `RESOLUCIÓN`, donde el diagnóstico único *es* el veredicto tras el certificado de finalización de fases; fijarlo antes sería diseñar en abstracto. Detalle en #711 y #765. Fuera de foco: ADR-021 y #644-648, aparcados.
