...
| Id | Necesidad | % Cobertura | Qué falta |
|---|---|---|---|
| N004 | Incorporar documentos al expediente, con organización automática en ruta predeterminada y registro de su localización | 50% | La organización automática en carpeta solo existe para documentos generados por el propio sistema (escritos/certificados); lo que Tramitador/Administrativo incorporan a mano solo se registra donde ya estuviera, sin mover ni copiar nada. El cálculo de la carpeta ESFTT destino y de `AT-N/pool/` ya existe como servicio (`app/services/rutas_esftt.py`, ADR-032) pero ningún endpoint de ingesta ni el alta de vínculo a tarea lo invoca todavía — no mueve nada en la práctica. |
...
| N009 | Expediente documental reconstruible sin BDDAT — estructura predecible fuera de BD | 25% | La convención de carpeta predecible solo se aplica a documentos generados por el sistema. El cálculo de ruta ESFTT legible a partir de códigos inmutables de catálogo ya existe (`app/services/rutas_esftt.py`, ADR-032) pero está desconectado: nada lo invoca todavía, así que no mueve ni organiza ningún fichero real. Falta conectarlo (mover al vincular a tarea, ingesta multipart al pool), forzar la convención también en la incorporación manual, y generar un manifest/índice en disco por expediente — sin BD, una carpeta es hoy un montón de ficheros sin diferenciar. |
