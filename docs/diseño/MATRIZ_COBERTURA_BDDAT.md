...
|---|---|---|---|
| N001 | Crear, editar, avanzar y cerrar Solicitudes/Fases/Trámites/Tareas de un expediente | 93% | CRUD completo de los 4 niveles ESFTT + isla React (ADR-016). El "avanzar" de la tarea ANALIZAR con su checklist de diagnóstico (documental #495 + técnico #581 + requerimientos #440 → `Diagnostico.defectos` #442) está verificado de extremo a extremo (2026-08-01): la batería de bugs de UX/datos hallada el 2026-07-22 está cerrada (recarga inline en vez de remount completo #693/#697, checklist condicionado por tipo de trámite #695, shuttle sin el 422 de `texto_libre` #701, icono de editar deshabilitado en ítem libre persistido #700, Guardar/Cancelar de campos directos unificado en la cabecera #688/#694, alineación del botón de diagnóstico #696). El vínculo `CONSUMIDO` que sostiene el peldaño 3 de ADR-033 §5 (revertir un diagnóstico ya notificado) es ahora real, no letra muerta: se deriva automáticamente al vincular el producido del ELABORAR, acreditado con el código de trazabilidad de #182 (#711, #714, #717) — sin backfill de expedientes anteriores a #182, protegidos por el criterio más estricto de #711. El borrado del árbol ya no es una vía alternativa para perder esa misma evidencia (#722, ver N003). Queda un frente abierto, más estructural que el anterior: una fase cerrada no está sellada — su diagnóstico de cierre sigue siendo reversible y `resultado_fase_id` recambiable sin recheck en la mayoría de los caminos (#720, absorbe #716); el orden canónico de tareas no gobierna la creación (#719); falta fricción simétrica al desdecirse de lo ya exigido al titular (#724). |
...
| N003 | Borrado de elementos ESFTT condicionado por el motor de reglas | 55% | Desde #722, el invariante estructural (no borrar evidencia de un acto ya comunicado) corre de verdad en el camino vivo — antes era código muerto: `check_invariante('BORRAR', ...)` conectado a `mutaciones_arbol.borrar_*`, bloquea sin excepción una TAREA con `Notificacion` vinculada (mismo criterio LPACAP que #714) y exige TRAMITE/FASE/SOLICITUD vacíos (hoja a hoja, sin la cascada manual que antes podía llevarse evidencia de rebote). Lo que da nombre a la necesidad sigue sin existir: cero reglas `BORRAR` en `reglas_motor` (24 reglas activas, todas `CREAR` — sin criterio de negocio dado, decisión explícita de aplazarlo) y ningún bypass por justificación cableado desde `api_expedientes.py::borrar_nodo` (nada escapable hoy, también decisión explícita). `api_bc.py` sigue montado como blueprint activo pese a documentarse como código muerto — ver Hallazgo 4 de `DETALLE_NECESIDADES_BDDAT.md`. |
...
| N084 | Extraer datos estructurados de un justificante de notificación externo (fecha, estado) para reducir el registro manual | 80% | Parser de Notifica-PNT completo y validado (`app/services/parser_justificante_notifica.py`, #655): PDF y ZIP (Informe.pdf + InformeENI.xml), con cotejo de coherencia PDF↔XML. Desde #657/#658 (ADR-034, corrige ADR-008), interfaz completa de la tarea NOTIFICAR (`NotificarEditor.jsx`, patrón `AnalizarEditor`/`ElaborarEditor`): "Registrar envío" (canal + `identificador_envio` + fecha de puesta a disposición, con adjunto opcional para autorrelleno transitorio — nunca se persiste como `Documento`) y "Completar resultado" (resultado + fecha + intento + observaciones). `notificaciones` pasa a tabla de seguimiento anclada a `tarea_id` (ya no vitaminado de documento 1:1). Cotejo automático y no bloqueante de `identificador_envio` en el hook de `editar_tarea` al fijar el documento producido. Verificado end-to-end sobre AT-2004. Falta: BandeJA y SIR sin parseo automático — sin muestras reales para construirlo, el registro en esos dos canales sigue siendo manual dentro del propio formulario. #712 (abierto) es un refinamiento del vínculo en dos actos, no un hueco nuevo. |
...
| N012 | Generar escrito desde plantilla y descargar versión borrador/firmada | 97% | Ciclo completo verificado (#608): generar desde plantilla, preview, vinculación automática como consumido, ciclo hasta PENDIENTE_FIRMA. `ContextoSubsanacion` separa ya los defectos del `Diagnostico` congelado en tres tokens por origen (documental/técnico/libre) para la plantilla de requerimiento. Desde #726 (ADR-035), motor de render `.odt` propio, hermano del `.docx`, elegido por la extensión de `plantilla.ruta_plantilla` sin columna nueva — mismo contrato de entrada/salida, sin dependencias añadidas. La ruta generada ya no mezcla separadores `/`/`\` (#699). Trazabilidad por token embebido construida y en uso real (#182 — detalle en N009), aunque su único consumidor hoy es #717 (N001); la inspección automática genérica (#181) sigue sin construir. Falta: conversión DOCX→PDF manual fuera de BDDAT (no prioritario); nombre sugerido con "ANY" cuando `nombre_en_plantilla` es NULL en algún nivel ESFTT (#698, aplazado — es poblado de catálogo, no bug de código). |
...
| Id | Necesidad | % Cobertura | Qué falta |
|---|---|---|---|
| N015 | Validación de flujo en tiempo real: qué se puede crear/iniciar/cerrar y cuándo, sin bloqueo silencioso | 75% | El motor evalúa y nunca degrada a "permitido" en silencio; los bloqueos llegan con motivo y norma legibles (HTTP 422 explícito), y los invariantes de cierre están hardcoded con mensajes propios — matizado por #716/#720 (absorbido en #720): `_check_cierre_fase` solo corre en la ventana estrecha de fijar el documento de resultado por primera vez, así que guardar el resultado sin documento adjunto lo esquiva del todo (reproducido en 4 fases reales), y una vez fijado el documento, el resultado se puede recambiar sin volver a pasar por el check. La fase cerrada no queda sellada frente a esas dos vías (a diferencia del borrado, ya cerrado por #722 — ver N003). |
