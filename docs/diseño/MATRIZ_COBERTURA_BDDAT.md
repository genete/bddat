...
| N078 | Mantener el catálogo de requisitos documentales exigidos por normativa, con condiciones de aplicabilidad según el trámite/instalación | 80% | CRUD completo (#583/#584: modelo `RequisitoDocumental`/`CondicionRequisito`/`DocumentoRequisito`, módulo `admin_requisitos`). El motor que evalúa las condiciones (`app/services/requisitos.py`) soporta los 12 operadores del catálogo compartido (#601/#660 corregidos — antes solo 6, GT/GTE/LT/LTE/BETWEEN/NOT_BETWEEN se ignoraban en silencio). Contenido normativo real sin poblar (#408 abierto). |
