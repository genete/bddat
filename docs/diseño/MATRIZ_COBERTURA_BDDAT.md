...
|---|---|---|---|
| N001 | Crear, editar, avanzar y cerrar Solicitudes/Fases/Trámites/Tareas de un expediente | 97% | Nada funcionalmente relevante — CRUD completo de los 4 niveles ESFTT + isla React (ADR-016). El "avanzar" de la tarea ANALIZAR queda completo con su checklist de diagnóstico: documental (#495) + técnico (#581) + requerimientos (#440) → `Diagnostico.defectos` (#442), único documento de salida. Los tres ejes son ahora continuos por solicitud entre vueltas de subsanación, no por tarea (#679, ADR-033 §7). Verificación manual de extremo a extremo (2026-07-22) sobre el ciclo completo hasta la 2ª tarea ANALIZAR encontró varios bugs de UX/datos no bloqueantes para el % (recarga completa del inspector en cada mutación, checklist compartido visible entre vueltas, validación del shuttle rota tras recargar) — issues abiertos, no corregidos aún. |
...
| N009 | Expediente documental reconstruible sin BDDAT — estructura predecible fuera de BD | 60% | Desde #667, la carpeta ESFTT legible (derivada de códigos de catálogo inmutables) se aplica de verdad al vincular un documento a una tarea, para cualquier mecanismo de entrada — ya no es solo cálculo desconectado. Falta: los escritos generados por el sistema siguen en la convención plana antigua (#183, ver N004), y no existe ningún manifest/índice en disco por expediente — sin BD, hay que inferir qué es cada fichero por la propia ruta (códigos de catálogo, no texto libre), no por un índice explícito. |
...
| N084 | Extraer datos estructurados de un justificante de notificación externo (fecha, estado) para reducir el registro manual | 35% | Parser de Notifica-PNT completo y validado (`app/services/parser_justificante_notifica.py`, #655): PDF y ZIP (Informe.pdf + InformeENI.xml), con cotejo de coherencia PDF↔XML. Sin ninguna integración en interfaz — no hay ningún punto de la aplicación que lo llame, el registro sigue siendo 100% manual (#657). BandeJA y SIR sin cobertura, sin muestras reales. Sin cotejo contra `id_remesa` persistida para detectar asociación a expediente equivocado — ni siquiera existe dónde guardar la remesa (#658). |
...
| N012 | Generar escrito desde plantilla y descargar versión borrador/firmada | 95% | Ciclo completo verificado (#608): generar desde plantilla, preview, vinculación automática como consumido, ciclo hasta PENDIENTE_FIRMA. `ContextoSubsanacion` separa ya los defectos del `Diagnostico` congelado en tres tokens por origen (documental/técnico/libre) para la plantilla de requerimiento. Falta: conversión DOCX→PDF manual fuera de BDDAT (no prioritario); trazabilidad por token embebido sin implementar (#181/#182); dos bugs encontrados en verificación manual (2026-07-22) — nombre sugerido con "ANY" cuando `nombre_en_plantilla` es NULL en algún nivel ESFTT, y ruta resultante con separadores `/`/`\` mezclados por config con `/` + `os.path.join` — issues abiertos, no corregidos. |
