...
| Id | Necesidad | % Cobertura | Qué falta |
|---|---|---|---|
| N004 | Incorporar documentos al expediente, con organización automática en ruta predeterminada y registro de su localización | 65% | La ingesta multipart (#666) ya copia y organiza en `AT-N/pool/<prefijo-hash>_<nombre>` con naming determinista sin colisiones. Falta: el registro in situ (explorador de servidor) sigue sin mover/copiar nada, solo registra donde ya estuviera; y la organización final en la carpeta ESFTT legible (derivada de códigos de catálogo, no solo `pool/`) solo debe ocurrir al vincularse a una tarea — ese movimiento todavía no está conectado (#667). |
...
| N009 | Expediente documental reconstruible sin BDDAT — estructura predecible fuera de BD | 35% | La convención de carpeta predecible por códigos de catálogo se aplica a los documentos generados por el sistema y, desde #666, también a la entrada al pool (`AT-N/pool/<prefijo-hash>_<nombre>`, naming determinista, sin colisiones). El cálculo de la carpeta ESFTT legible en sí (`app/services/rutas_esftt.py`, ADR-032) sigue desconectado del movimiento real de ficheros: falta conectarlo al vincular a tarea (#667), forzar la convención también en el registro in situ, y generar un manifest/índice en disco por expediente — sin BD, una carpeta es hoy un montón de ficheros sin diferenciar. |
