...
| N082 | Mantener el catálogo de tipos de documento (código identificador inmutable tras el alta, usado en el motor de reglas y en el código, mismo régimen que los tipos ESFTT de N019) | 0% | Catálogo poblado y en uso real (motor de reglas, criterio `EXISTE_DOCUMENTO_TIPO`; picklist de solo lectura en `tablas_maestras` para el editor de pasos del trámite) — pero cero interfaz de alta/edición/baja. El único canal de escritura es la migración Alembic (#377); #337 dejó la UI de gestión del catálogo fuera de alcance de forma explícita. |
