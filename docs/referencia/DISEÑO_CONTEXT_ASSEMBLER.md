...
| `requiere_aau` | boolean | `dato` · Solicitud | **D356/2010 art. 2.1.a** (GICA) · DL 26/2021 DF 4ª · **Ley 2/2026 art. 67** — la instalación requiere autorización ambiental unificada | definida |
...
| `tiene_punto_acceso_conexion` | boolean | `calculado` · Solicitud | RD-ley 23/2020 art. 1 — cobertura del RequisitoDocumental PERMISO_ACCESO_CONEXION (RD 1183/2020 art. 3) vía `documentos_requisito` para la solicitud en contexto; condición de admisión a trámite de AAP renovables, acotada por `tipo_expediente='Renovable'`. **Desviación de naturaleza** respecto a lo publicado originalmente: se implementó como `calculado` (mismo patrón que `tasa_impagada`, #582) en vez de `derivado_documento` puro — reutiliza el checklist de requisitos ya construido en #495/#583 en lugar de una consulta cruda de existencia de documento en el pool | implementada en `calculado.py` (#780) |
