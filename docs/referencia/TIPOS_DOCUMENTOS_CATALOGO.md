...
| `DOC_PROYECTO` | Proyecto técnico del titular | EXTERNO | REDACTAR_ANUNCIO.ELABORAR (consumido) | | Proxy de entrada: aporta el contexto y características del proyecto necesarios para redactar el anuncio. Los DOC_PROYECTO del expediente forman la línea temporal del proyecto; el que abre versión nueva tiene fila en `reformados_proyecto` (ADR-044 §C). **Único tipo con `fecha_administrativa` obligatoria** (#885): es la que ordena las versiones. REDACTAR_ANUNCIO es trámite nuevo pendiente de añadir al FTT (también falta ANUNCIO_BOJA) |
...
| `CERT_PLAZO_TABLON` | Certificado de exposición en tablón de ayuntamiento | EXTERNO | TABLON_AYUNTAMIENTOS.ESPERAR_PLAZO (producido) | | Emitido por el ayuntamiento una vez finalizado el período de exposición. `fecha_administrativa` = **fecha de inicio de la exposición** (no la fecha de recepción del certificado ni la de su firma — es la fecha que el ayuntamiento hace constar como inicio del período). El motor la usa como referencia para calcular `fin = fecha_administrativa + 30 días naturales` (art. 125 RD 1955/2000). El ESPERAR_PLAZO queda COMPLETADO retroactivamente al incorporar el certificado. Ver #416. |
