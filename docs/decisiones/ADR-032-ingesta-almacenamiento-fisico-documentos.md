...
**Estado:** Adoptada — **sobrepasada por ADR-050** (2026-09-25) en §1-§4, pendiente de implementar. Hasta que se implemente, este ADR describe lo que hay en el código
**Sobrepasada por:** [ADR-050](ADR-050-almacen-documental-privado-por-contenido.md) — BDDAT pasa a ser el único dueño de los ficheros, en un almacén privado direccionado por contenido. §1: el registro in situ lo sustituye un buzón, y toda entrada copia al almacén. §2: `Documento.url` deja de guardar rutas locales; la ficha apunta al fichero del almacén por su hash (`documentos.fichero_sha256`). §3: vincular a una tarea ya no mueve nada, es solo una fila en BD. §4: el naming con prefijo MD5 en `pool/` lo sustituye el SHA-256 como nombre en el almacén. La carpeta ESFTT legible sobrevive como exportación y manifiesto (ADR-050 §H), no como almacenamiento
**Fecha:** 2026-07-16
...
Patrón seguro de movimiento: copiar a destino → actualizar `Documento.url` y hacer `commit` → borrar el origen solo tras `commit` exitoso. Si el `commit` falla, queda como mucho una copia huérfana en destino (limpiable sin urgencia) y el documento nunca pierde su referencia válida. Colisión de nombre en la carpeta ESFTT destino (dos documentos distintos que coinciden en nombre original, caso raro): se sufija con el id del documento (`nombre_<id>.ext`), sin tocar el naming por hash del pool.

**Resuelto en la implementación (#667):** sí, vuelve a `pool/` — al perder su último vínculo (desvinculación, borrado de tarea), el documento se mueve de vuelta a `AT-N/pool/` con el mismo patrón seguro copiar→commit→borrar. Mantiene el invariante de ADR-027 §2 (`pool/` = espejo fiel del estado huérfano; carpeta ESFTT = espejo de vinculado) en las dos direcciones, no solo al entrar.

**Corrección de diseño encontrada durante la implementación (#926):** el patrón "copiar → commit → borrar origen" asumía que el documento que se mueve es el único que apunta a ese origen. Un duplicado exacto (§4: mismo `hash_md5`, mismo `url`, dos filas `Documento`) lo rompe — mover uno borraba el fichero del que el otro seguía dependiendo, y su siguiente movimiento fallaba con el origen ya desaparecido. Los documentos que comparten fichero se mueven sin borrar el origen mientras otro `Documento` lo siga usando: el borrado solo ocurre en el movimiento que deja de tener a nadie apuntando a ese origen. Simétricamente, si el destino de un movimiento ya existe con el mismo contenido (mismo MD5), se reutiliza en vez de sufijarse con el id — dos documentos que comparten fichero en el pool lo siguen compartiendo también en la carpeta ESFTT (o de vuelta en `pool/`), en vez de duplicarlo físicamente. Con contenido distinto, la colisión de nombre sigue resolviéndose como antes (sufijo `_<id>`).

**Corrección de diseño encontrada durante la implementación (#667):** `editar_tarea()` (`app/services/mutaciones_arbol.py`) borraba y volvía a crear TODOS los vínculos `DocumentoTarea` de la tarea en cada guardado del formulario (`clear()` + recrear), incluso cuando el conjunto de documentos no cambiaba — un guardado que solo tocaba las notas ya generaba filas `DocumentoTarea` nuevas. Esto hacía indetectable por BD la "primera vinculación real" (criterio de este §3): el historial de filas no es fiable si se borra y recrea sin motivo. Se corrigió a un diff (borra solo lo que ya no está, crea solo lo nuevo, no toca lo que no cambia) — con eso, "el documento no tenía ningún vínculo antes de esta fila" (`documento.vinculos_tarea` vacío) vuelve a ser una pregunta fiable, tal como asume el párrafo anterior. Efecto colateral aceptado: `vinculos_documento` está `order_by=DocumentoTarea.id` (`app/models/tareas.py`), así que si el usuario solo reordena documentos consumidos sin añadir/quitar ninguno, el orden mostrado no cambia (antes sí, porque recrear daba ids nuevos en el orden recibido) — no hay hoy ninguna garantía de orden documentada que dependiera de ese efecto.
