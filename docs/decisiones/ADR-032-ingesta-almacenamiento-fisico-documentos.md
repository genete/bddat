...
**Sobrepasada por:** [ADR-050](ADR-050-almacen-documental-privado-por-contenido.md) — BDDAT pasa a ser el único dueño de los ficheros, en un almacén privado direccionado por contenido. §1: el registro in situ lo sustituye un buzón, y toda entrada copia al almacén. §2: `Documento.url` deja de guardar rutas locales; el fichero es una versión en `documento_versiones`. §3: vincular a una tarea ya no mueve nada, es solo una fila en BD. §4: el naming con prefijo MD5 en `pool/` lo sustituye el SHA-256 como nombre en el almacén. La carpeta ESFTT legible sobrevive como exportación y manifiesto (ADR-050 §H), no como almacenamiento
**Fecha:** 2026-07-16
