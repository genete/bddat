...
**Enmendado por:** ADR-051 (2026-09-26, sin implementar; lo implementa N5). §E: la notificación al titular es la `NOTIFICAR` de fuente `TITULAR` en el trámite `NOTIFICACION` de la fase que resuelve el acto, no cualquier `NOTIFICAR` de ese trámite (ADR-051 §F). «Lo que este ADR no decide», primer punto: cada justificante se liga a su destinatario por su tarea, una `NOTIFICAR` por destinatario, sin certificado de notificación múltiple (ADR-051 §A). Consecuencias: el destinatario de `notificaciones` es entidad + fuente + dirección copiada (ADR-051 §B)
...
| Qué protege | El documento citado, su vínculo con la tarea y la tarea (la fase sigue abierta) | Nada nuevo: el sellado de ADR-036 (y su propia foto fija, #956) |
...
- **Hecho en #956 (N4b):** `CERT_CIERRE_FASE` y `services/cert_cierre_fase.py` (revisar, emitir y deshacer). Cerrar una finalizadora es emitirlo: ocupa `documento_resultado_id` y `editar_fase` ya no deja fijar esa columna en ellas (el editor ni la ofrece). El informe «¿cómo voy?» redacta cada trámite con `informe_instruccion.bloque_tramite`; «obligatorio» es lo creado en la fase, completo, más el resultado y el certificado de cumplimiento (lo nunca creado lo cubrirá #805, que tiene el hueco preparado). Cuatro decisiones de su diseño enmiendan o precisan este apartado:
  - **Sin escape a nivel de fase.** Con cualquier pendiente no se emite, y no hay justificación que lo evite: es el modelo de `CERT_FIN_INSTRUCCION`, en el que cada escape se hace en su propio acto y nodo —la tarea, el trámite— y el certificado solo los lee y relata como salvados. Cerrar la finalizadora con un organismo o un interesado sin notificar es zona de nulidad (art. 47.1.e LPACAP) o de anulabilidad por indefensión (art. 48.2), por mucho que se justifique. Es una **excepción razonada al criterio de #723**, que reservó la puerta cerrada a cuando el acto ya salió fuera: aquí ya salió, porque se exige el certificado de cumplimiento. Y **enmienda la D4 de #928 para las finalizadoras**: la sede pendiente ya no se fuerza al cerrar la fase (`_check_completitud_cierre` queda solo para las no finalizadoras); queda el escape de su propia tarea, la sede «justificada», que el certificado relata como salvado (y también el de fin de instrucción, que comparte la redacción de la tarea).
  - **Foto fija.** Al emitir, `certificados.datos` guarda el informe ya redactado; la vista del emitido lo lee y no recalcula. Motivo: el sellado de ADR-036 no protege la fecha, el tipo ni el fichero de los documentos de la fase en el pool (#954).
  - **Confirmación con frase.** `_check_reabrir` no cambia, así que el certificado que cierra la última finalizadora —con la notificación al titular ya hecha— es irreversible dentro de BDDAT. Cuando la emisión deja la solicitud resuelta se pide escribir «cerrar finalizadora» en un modal bloqueante, y el backend lo exige también; si no la deja resuelta, basta una confirmación normal.
  - **Reabrir es deshacer.** `reabrir_fase` delega en el servicio en las finalizadoras: borra el certificado y su documento, vacía resultado y documento de resultado y deja en bitácora lo mismo que una reapertura más los ids retirados. El siguiente certificado lo relata.
