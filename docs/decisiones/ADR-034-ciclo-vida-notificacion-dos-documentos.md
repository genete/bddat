...
**Enmendado por:** ADR-051 §A/§B/§D (2026-09-26, sin implementar; lo implementa N5). La fila nace al crear la `NOTIFICAR`, no al vincular el primer justificante, y guarda el destinatario (entidad, fuente y dirección copiada); el hook deja de crear y de borrar filas. Sigue una fila por tarea: con varios destinatarios hay una `NOTIFICAR` por destinatario.
